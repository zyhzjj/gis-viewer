...
(function (global) {
  ...
  async function parseShpFromBuffers({ shpBuf, dbfBuf, prjText, name }) {
    const warnings = [];
    let geojson = null;
    ...
    if (dbfBuf) {
      try {
        props = await shp.parseDbf(dbfBuf);
      } catch (e) {
  ...
  async function parseShapefileGroup(group) {
    ...
    return parseShpFromBuffers({ shpBuf, dbfBuf, prjText, name: group.shp.name });
  }
  ...
  async function parseZip(file) {
    ...
    for (const grp of entries) {
      ...
      const r = await parseShpFromBuffers({ shpBuf, dbfBuf, prjText, name: file.name });
      merged.features = merged.features.concat(r.geojson.features || []);
      if (r.crs) { crs = r.crs; crsName = r.crsName; crsSource = r.crsSource; }
  ...
  async function parseGeoJSON(file) {
    ...
    if (gj.crs) {
      ...
      if (m) {
        ...
        else warnings.push(`GeoJSON 声明坐标系 ${code}，但缺少转换参数`);
      }
    }
  ...
  async function parseCSV(file) {
    ...
    let lonIdx = li, latIdx = la;
    // 表头没认出来时，按数值范围猜（经度 |x|<=180 且 纬度 |y|<=90）
    if (lonIdx < 0 || latIdx < 0) {
      warnings.push("未识别到标准经纬度列名，尝试按数值范围推断");
      const cand = [];
      for (let c = 0; c < header.length; c++) {
        const vals = rows.slice(1, 30).map(r => parseFloat(r[c])).filter(Number.isFinite);
        if (vals.length < 2) continue;
        const mn = Math.min(...vals), mx = Math.max(...vals);
        if (mn >= -180 && mx <= 180) cand.push({ c, mn, mx });
      }
      const lonC = cand.find(x => x.mn >= -180 && x.mx <= 180);
      const latC = cand.filter(x => x !== lonC).find(x => x.mn >= -90 && x.mx <= 90);
      if (lonC && latC) { lonIdx = lonC.c; latIdx = latC.c; }
    }
    ...
    for (let i = 1; i < rows.length; i++) {
      const r = rows[i];
      const lon = parseFloat(r[lonIdx]), lat = parseFloat(r[latIdx]);
      if (!Number.isFinite(lon) || !Number.isFinite(lat)) { skipped++; continue; }
      const props = {};
      header.forEach((h, c) => { if (c !== lonIdx && c !== latIdx) props[h] = r[c]; });
    ...
    return {
      ...
      crs: "EPSG:4326", crsName: "WGS 84 经纬度（假定）", crsSource: "default",
      warnings
    };
  }
  ...
  async function parseGeoTIFF(file) {
    ...
    if (rawCode && rawCode !== 32767) {
      ...
      if (!(await CRSUtil.ensureDef(crs))) {
        warnings.push(`GeoTIFF 声明坐标系 ${crs}，但缺少转换参数，按原坐标处理`);
        crs = null;
      }
    } else {
      warnings.push("GeoTIFF 未标注坐标系，按 WGS 84 经纬度处理");
    }
    ...
    if (isRGB) {
      ...
      const ranges = [];
      for (let b = 0; b < 3; b++) ranges.push(percentileRange(rasters[b]));
      stats = { mode: "rgb", ranges };
      for (let i = 0; i < rw * rh; i++) {
        let r, g, bl;
        if (nodata !== null && rasters[0][i] === nodata) {
      ...
      // 单波段：色带渲染 + 2%~98% 拉伸
      const range = percentileRange(rasters[0]);
      ...
      for (let i = 0; i < rw * rh; i++) {
        const v = rasters[0][i];
        if (nodata !== null && v === nodata) { out[i * 4 + 3] = 0; normData[i] = 0; continue; }
    ...
    // 把 bbox 从数据坐标系转换到 WGS84，得到 Leaflet 需要的 [[南,西],[北,东]]
    let bounds;
    if (crs && crs !== "EPSG:4326") {
      const sw = CRSUtil.toWgs84(bbox[0], bbox[1], crs);
      const ne = CRSUtil.toWgs84(bbox[2], bbox[3], crs);
    ...
    return {
      raster: {
        canvas, bounds, width, height, rw, rh,
        bands: samples, nodata, stats, resampled,
        ramp: isRGB ? null : "gray"
      },
      crs, crsName, warnings
    };
  }
  ...
  async function loadFiles(fileList, onProgress) {
    ...
    for (const g of groups) {
      try {
        ...
        [g.shp, g.dbf, g.prj, g.shx].forEach(f => f && consumed.add(f));
        datasets.push(Object.assign({
          id: uid(), name: g.shp.name, kind: "vector", properties: r.geojson
